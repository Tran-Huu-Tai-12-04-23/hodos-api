  ...
  async findRoute(body: FindRouteDTO) {
    try {
      const url =
        ...
        `?apiKey=${this.VIETMAP_API_KEY}&optimize=true&points_encoded=false&avoid_return=true&point=${body?.origin.lat},${body?.origin.lng}&point=${body?.destination.lat},${body?.destination.lng}`;
